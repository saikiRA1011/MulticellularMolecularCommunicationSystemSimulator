 ...
 * @param z (default: 0)
 */
Vec3::Vec3(double x, double y, double z = 0)
{
    this->x = x;
    this->y = y;
    this->z = z;
...
Vec3 Vec3::rotate(double theta, double phi) const noexcept
{
    ...
    double rotateZ[3][3] = { { std::cos(theta), -std::sin(theta), 0 },
                             { std::sin(theta), std::cos(theta), 0 },
                             { 0, 0, 1 } };
    double rotateY[3][3] = { { std::cos(phi), 0, std::sin(phi) },
                             { 0, 1, 0 },
...
{
    Vec3 tmp(y * vec.z - z * vec.y, z * vec.x - x * vec.z,
             x * vec.y - y * vec.x);
